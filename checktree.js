(function() {
  ...
  class TreeOption {
    ...
    _addChild(child, autoOpen) {
      ...
      if (Array.isArray(child.children) && child.children.length > 0) {
        ...
        newChild.addChildren(child.children, autoOpen);
  ...
  function getTreeOptionFromId(root, id) {
    ...
    id.substr(2).split('.').some(
      function(key) {
        obj = obj.children[parseInt(key,10)];
  ...
  class CheckTree extends HTMLElement {
    ...
    setChecked(nodeId, checked) {
      var treeOption = getTreeOptionFromId(this._root, nodeId);
      treeOption.checked = checked;
    }
